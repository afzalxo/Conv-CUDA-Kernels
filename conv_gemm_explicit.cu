...
void print_3d_tensor(std::vector<int> a, int rows, int cols, int channels){
	printf("\n");
...
void print_4d_tensor(std::vector<int> a, int rows, int cols, int channels, int number){
	printf("\n");
...
int main(){
	...
	std::string const fname = {"trace_conv_gemm_explicit.csv"};
	int dev = 0;
	...
	vector<int> h_kern(k*k*C*K);
	vector<int> h_feat(H*W*C);
	vector<int> h_mat_res(feat_tr_H*K);
	generate(h_kern.begin(), h_kern.end(), []() {return rand() % 100; });
	generate(h_feat.begin(), h_feat.end(), []() {return rand() % 100; });
	...
	logger.caller_state = 0; //Host to Mem Transfer
	gpuErrchk(cudaMalloc(&kern, k*k*C*K*sizeof(int)));
	gpuErrchk(cudaMalloc(&feat, H*W*C*sizeof(int)));
	gpuErrchk(cudaMalloc(&kern_tr, k*k*C*K*sizeof(int)));
	gpuErrchk(cudaMalloc(&feat_tr, feat_tr_H*feat_tr_W*sizeof(int)));
	gpuErrchk(cudaMalloc(&mat_res, feat_tr_H*K*sizeof(int)));

	gpuErrchk(cudaMemcpy(kern, h_kern.data(), k*k*C*K*sizeof(int), cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(feat, h_feat.data(), H*W*C*sizeof(int), cudaMemcpyHostToDevice));
	...
	print_4d_tensor(h_kern, k, k, C, K);
	...
	gpuErrchk(cudaMemcpy(h_mat_res.data(), mat_res, feat_tr_H*K*sizeof(int), cudaMemcpyDeviceToHost));
	gpuErrchk(cudaDeviceSynchronize());
	logger.caller_state = 5; //Finished DevToHost state.
	std::thread threadKill(&NVMLMonThread::killThread, &logger);
	threadStart.join();
	threadKill.join();
#if DEBUG
	printf("\nPrinting results\n");
	print_3d_tensor(h_mat_res, feat_tr_H, K, 1);
#endif
	cudaFree(kern);
	cudaFree(feat);
	cudaFree(feat_tr);
